...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, indices,rank,size, data_ID, data_path ,cache_size, to_load, local_batch_size,ratio=0.9, dataset='all', load_all=False, scale_I=0, shuffle=False):
    ...
    def getItemBalancing(self,idx,flag):
        ...
        if not cached and not prefetched:
            ...
            X_train = diff[np.newaxis]
            Y_I_train = amp[np.newaxis]
            Y_phi_train = phi[np.newaxis]
            x=torch.Tensor(X_train)
            y1=torch.Tensor(Y_I_train)
            y2=torch.Tensor(Y_phi_train)
            if flag:
...
def swift_collate(batch):
    ...
    for xx in batch:
        if xx[0] is not None:
            numel += len(xx[0])
    tensor_x = torch.zeros(size=(numel,1,64,64,64))
    tensor_y1 = torch.zeros(size=(numel,1,64,64,64))
    tensor_y2 = torch.zeros(size=(numel,1,64,64,64))
    pointer=0
...
nepochs=100
#cache_size=50000
#cache_size=21140#8gb*8proc
cache_size=42282#16gb*8proc
...
train_data2=Dataset(indices=shuffle_list_sorted,
                    rank=rank,
                    size=size,
                    data_ID=DATA_ID, 
                    data_path=DATA_PATH,
